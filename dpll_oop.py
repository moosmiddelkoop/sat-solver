...
class SATsolver:
    def __init__(self, clauses, heuristic, sudoku_size = 9):
        self.clauses = clauses
        self.heuristic = heuristic
        self.sudoku_size = sudoku_size
        self.backtracks = 0

    def get_vars(self, clauses):
        '''
    ...
    def simplify_clauses(self, clauses, variable):
        """
    ...
    def solve(self, clauses, var_dict):
        """
        ...
        # handle unit clauses
        temp_clauses, temp_vars = self.handle_unit(temp_clauses, temp_vars)

        self.check_result(temp_clauses)
        if self.result != None: 
            return self.result, temp_vars
        
        # Threshold for human heuristic: if more than 60% of Sudoku filled, do not apply heuristic
        if self.heuristic == 'human':
            no_true_literals = len([k for k,v in temp_vars.items() if v == True])
            if no_true_literals > (self.sudoku_size ** 2) * 0.7:
                self.heuristic = None
        ...
        if self.heuristic == None:
            ...
            temp_clauses = self.simplify_clauses(temp_clauses, variable)
            self.result = self.solve(temp_clauses, temp_vars)
            if(self.result): 
                return self.result
            ...
            variable = next_literal
            temp_clauses = self.simplify_clauses(temp_clauses, variable)
            return self.solve(temp_clauses, temp_vars)

        #After handling unit clause, apply heuristics
        elif self.heuristic == 'human':

            key, assignment = self.human_heuristic(temp_vars, self.sudoku_size)
            ...
            key, assignment = self.dlcs_heuristic(temp_clauses, temp_vars)
            temp_vars[key] = assignment
            variable = key if assignment else -key
            temp_clauses = self.simplify_clauses(temp_clauses, variable)
            self.result = self.solve(temp_clauses, temp_vars)
    ...
    def handle_unit(self, clauses, var_dict):
        '''
        ...
        while i < len(clauses):
            ...
            if len(clauses[i]) == 1:
                ...
                clauses = self.simplify_clauses(clauses, variable)
    ...
    def dpll(self):
        ...
        '''
        # get variable dict
        variables = self.get_vars(self.clauses)   

        # Get sudoku size (max number of rows as proxy): Needed for human heuristic
        self.sudoku_size = max(int(str(item)[0]) for item in list(variables.keys()))
        
        # check if problem is already solved 
        # TODO: fix this
        self.result = self.check_result(self.clauses)
        if self.result != None: 
            return self.result, variables

        # solve it
        return self.solve(self.clauses, variables)


    def jeroslaw_heuristic(self, clauses, vars_dict):
    ...
    def human_heuristic(self, vars_dict, sudoku_size):
